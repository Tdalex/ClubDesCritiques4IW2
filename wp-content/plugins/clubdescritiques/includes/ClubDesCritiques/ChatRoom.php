...
class ChatRoom{
    ...
    public static function getNextChat(){
        ...
        $query = "SELECT ". $wpdb->posts .".ID, ".$wpdb->posts .".post_title  FROM ". $wpdb->posts ." INNER JOIN ". $wpdb->postmeta ." as metaA ON (". $wpdb->posts .".ID = metaA.post_id) WHERE cast(metaA.meta_value as datetime) >= now() AND metaA.meta_key = 'end_date' and ". $wpdb->posts .".post_status='publish' and ". $wpdb->posts .".post_parent=0 order by cast(metaA.meta_value as date) ASC";
